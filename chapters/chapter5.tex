...
Interakcja bazuje na bezpośrednim dotyku i manipulacji obiektami. Każdej akcji towarzyszy zsynchronizowane z nią wielozmysłowe informacje zwrotne które istotnie wzmacnia realizm interakcji i poczucie obecności w VR. Wynika to z analizy heurystycznej oraz z wniosków przedstawionych w literaturze \cite{Prototypy-XR} \cite{Projektowanie-VR}
...
Najprostszą formą interfejsu diegetycznego jest umieszczenie na scenie modelu urządzenia wyposażonego w ekran, na przykład komputera. Ekran takiego obiektu stanowi idealną bazę pod komponent \texttt{Canvas} w trybie \textit{World Space} dopasowany do ekranu urządzenia. Renderowanie elementów UI odbywa się identycznie jak w przypadku interfejsu 2D, jednak kontekst przestrzenny (fizyczna obecność urządzenia, konieczność podejścia do niego) nadaje interakcji charakter diegetyczny. Interakcja może odbywać się zarówno za pomocą promienia wskazującego z~dystansu, jak i poprzez bezpośrednie wskazywanie wirtualnym palcem.
...
System windy oparty jest na globalnym managerze przechowywanym w specjalnym stanie gry \textit{DontDestroyOnLoad}, dzięki czemu obiekt zostaje zachowany podczas przechodzenia między scenami. Gotowy prefabrykat zawiera również model wnętrza windy bez drzwi. Po załadowaniu nowego poziomu, instancja klasy \textit{Elevator} wyszukuje na scenie obiektu klasy \textit{ElevatorDock} o typie \textit{Start} i podłącza się do niego. \textit{ElevatorDock} jest komponentem osadzonym w osobnym prefabie, stanowiącym front windy z animowanymi drzwiami i to on zarządza logiką ich otwierania oraz zamykania. \textit{ElevatorManager} jedynie pozycjonuje się względem odpowiedniego doku.
