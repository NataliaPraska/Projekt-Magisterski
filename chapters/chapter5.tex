...
Głównym celem drugiego zadania była ocena intensywności informacji zwrotnej, a nie formy jej prezentacji. Przy projektowaniu uwzględniono naturalne ograniczenia pamięci użytkowników. W obu wersjach interfejs został umieszczony w środowisku wirtualnym, z zachowaniem identycznego sposobu poruszania się, aby w jak największym stopniu skupić uwagę użytkownika na analizowanym obszarze. Zadanie również nie uległo żadnym zmianom, a jedyną różnicą miedzy nimi była ilość wykorzystywanych kanałów feedbacku oraz ich intensywność. W wariancie A informacja zwrotna ograniczona jest do pojedynczego, nisko-intensywnego kanału dźwiękowego, natomiast w wariancie B zastosowano informację zwrotną wielokanałową, łączącą bodźce wizualne, dźwiękowe oraz haptyczne. Dodatkowo konstrukcja zadania została celowo zaprojektowana tak, aby użytkownik musiał podjąć decyzję „zgadywać” albo „szukać informacji w otoczeniu”. W tym celu miejsce wprowadzania kombinacji oraz miejsce odczytania wskazówki rozdzielono przestrzennie: wskazówkę umieszczono w osobnym pomieszczeniu, natomiast panel/element służący do wpisania hasła znajdował się w głównej przestrzeni zadania. Zabieg ten pozwalał ocenić, czy intensywność informacji zwrotnej wpływa na moment przerwania strategii prób i przejścia do poszukiwania poprawnej odpowiedzi. Zakładano trzy typowe przebiegi: (1) użytkownik kontynuuje wpisywanie kombinacji metodą prób, (2) po pierwszych nieudanych próbach, na podstawie informacji zwrotnej, orientuje się, że poprawne hasło powinno być gdzieś podane w środowisku i po odnalezieniu wskazówki wpisuje je poprawnie od razu, lub (3) odnajduje wskazówkę w osobnym pomieszczeniu, jednak nie zapamiętuje jej w pełni i dopiero przy kolejnym podejściu koryguje brakujące elementy, wpisując poprawną kombinację za drugim razem. Pozwalało to na ocenę wpływu niskiej i wysokiej intensywności feedbacku na przebieg i prędkość wykonania zadania.

\subsubsection{Zadanie 3: Lokomocja po platformach}

Zadanie trzecie skupiało się na mechanizmach lokomocji. Użytkownik przemieszcza się po torze przeszkód złożonym z niewielkich platform. W wariancie A zastosowano lokomocje ciągłą, natomiast w wariancie B teleportacje punktową. W zadaniu tym analizowany jest nie tylko czas potrzebny na ukończenie planszy, ale również liczba upadków. Zastosowane metryki miały pozwolić na ocenę stabilności i bezpieczeństwa wybranego sposobu przemieszczania się. Daje także możliwość ewaluacji wpływu na tempo poruszania się użytkownika.
